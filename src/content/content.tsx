...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'showOTP') {
    console.log('Showing OTP:', message.data.otp)
    if (!root) {
      injectApp()
    }
    console.log('Rendering ContentApp with OTP:', message.data.otp)
    root?.render(<ContentApp otp={message.data.otp} />)
